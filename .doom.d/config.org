...
(after! ivy
  ...
  (defun my--ivy-enter-directory-or-insert ()
    (interactive)
    (cond ((< (point) (length (thing-at-point 'line t))) (forward-char))
          ((my--ivy-is-directory-p) (counsel-down-directory))
          (t (let ((last-input (ivy--input)))
               (ivy-insert-current)
               (when (string= last-input (ivy--input))
                 (ivy-call))))))

  ;; M-o show some weird hydra
  ;; C-o show action list
  ;; C-M-o like M-o but does not quit
  ;; C-M-m ivy-call - does the action, but does not quit
