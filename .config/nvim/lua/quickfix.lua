...
local syntax_template = [[
  ...
  syn match qfHiddenWithPath '^%s' nextgroup=qfPath cchar= conceal
  syn match qfHiddenWithoutPath '^%s' cchar= conceal
...
function M.format(info)
  ...
  for list_index = info.start_idx, info.end_idx do
    ...
    if item then
      ...
      -- this one is here so it does not intefere with quickfix | used as separator
      text = fn.substitute(text, '|', '⏐', 'g')
...
function M.setup()
  ...
  vim.cmd([[
    function! PqfQuickfixTextFunc(info)
      return luaeval('require("quickfix").format(_A)', a:info)
    endfunction
  ]])
